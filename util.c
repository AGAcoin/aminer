	...
	curlsocktype purpose)
{
	...
	if (unlikely(setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &reuseaddr, sizeof(reuseaddr))))
	   return 1;
...
static bool send_line(struct stratum_ctx *sctx, char *s)
{
	...
	while (len > 0) {
		...
		if (SysNetWait(sctx->pNetData, false, sctx->sock, 0, 0) < 1)
		{
			return false;
        }
...
bool stratum_send_line(struct stratum_ctx *sctx, char *s)
{
    ...
    LockMutex(sctx->pSock_lock);
	ret = send_line(sctx, s);
    UnlockMutex(sctx->pSock_lock);

	return ret;
}

bool socket_full(struct SysNetData *pNetData, curl_socket_t sock, int timeout)
{
	if (SysNetWait(pNetData, true, sock, timeout, 0) > 0)
	{
		return true;
    }
    else
...
int socket_full_usec(struct SysNetData *pNetData, curl_socket_t sock, int timeout)
{
    return SysNetWait(pNetData, true, sock, 0, timeout);
}

bool stratum_socket_full(struct stratum_ctx *sctx, int timeout)
{
	return strlen(sctx->sockbuf) || socket_full(sctx->pNetData, sctx->sock, timeout);
}

bool stratum_socket_full_usec(struct stratum_ctx *sctx, int timeout)
{
    ...
    {
        return socket_full_usec(sctx->pNetData, sctx->sock, timeout);
    }
}
...
char *stratum_recv_line(struct stratum_ctx *sctx)
{
	...
	if (!strstr(sctx->sockbuf, "\n")) {
		...
		time(&rstart);
		if (!socket_full(sctx->pNetData, sctx->sock, 60)) {
		...
		do {
			...
			if (rc != CURLE_OK)
            {
				if (rc != CURLE_AGAIN || !socket_full(sctx->pNetData, sctx->sock, 1)) 
                {
#else
			...
			if (n < 0) {
				if (!socket_blocks() || !socket_full(sctx->pNetData, sctx->sock, 1))
                {
#endif
					ret = false;
					break;
	...
	struct curl_sockaddr *addr)
{
    ...
    stratum.pSocket = SysNetCreateSocket(addr->family, addr->socktype, addr->protocol);
    
    if (stratum.pSocket)
    {
        *sock = stratum.pSocket->s;
    }
...
bool stratum_connect(struct stratum_ctx *sctx, const char *url)
{
    ...
    sprintf(sctx->curl_url, "%s", url + 14);
    
    char            *pch = strrchr(sctx->curl_url, ':');
    ...
    address = SysNetResolveName(sctx->curl_url);

	//applog(LOG_INFO, "%s %d.%d.%d.%d : %d\n", sctx->curl_url, address & 0xff, (address & 0xff00) >> 8, (address & 0xff0000) >> 16, (address & 0xff000000) >> 24, port);
	sprintf(sctx->curl_url, "http://%d.%d.%d.%d:%d", address & 0xff, (address & 0xff00) >> 8, (address & 0xff0000) >> 16, (address & 0xff000000) >> 24, port);
...
start:
	...
	if (!socket_full(sctx->pNetData, sctx->sock, 30)) {
...
struct thread_q *tq_new(void)
{
    ...
    tq = AllocVec(sizeof(*tq), MEMF_ANY | MEMF_CLEAR);
	if (!tq)
		return NULL;

	INIT_LIST_HEAD(&tq->q);
	tq->pMutex = CreateMutex();
	tq->pCond = CreateCondition();
...
void tq_free(struct thread_q *tq)
{
    ...
    DestroyCondition(tq->pCond);
    DestroyMutex(tq->pMutex);
...
static void tq_freezethaw(struct thread_q *tq, bool frozen)
{
    ...
    SignalCondition(tq->pCond);
    UnlockMutex(tq->pMutex);
}
...
bool tq_push(struct thread_q *tq, void *data)
{
    ...
    ent = AllocVec(sizeof(*ent), MEMF_ANY | MEMF_CLEAR);
	if (!ent)
		return false;
...
void *tq_popNoWait(struct thread_q *tq)
{
	struct tq_ent *ent;
	void *rval = NULL;
...
void *tq_popWait(struct thread_q *tq)
{
    struct tq_ent *ent;
	void *rval = NULL;
	int rc;
	...
	rc = !WaitCondition(tq->pCond, tq->pMutex);
	if (rc)
		goto out;
	if (list_empty(&tq->q))
		goto out;
...
struct SysNetData *SysNetInit()
{
	struct SysNetData *pNetData;

    pNetData = AllocVec(sizeof(*pNetData), MEMF_ANY);
    if (pNetData)
    {
        ...
        if (SocketBase)
        {
            ...
            if (SocketBaseTags(SBTM_SETVAL(SBTC_ERRNOPTR(sizeof(errno))), (IPTR)&errno, SBTM_SETVAL(SBTC_HERRNOLONGPTR), (IPTR)&pNetData->herrno, TAG_DONE))
            {
                FreeVec(pNetData);
            ...
            pNetData->pTimerPort = CreateMsgPort();
            if (pNetData->pTimerPort)
            {
                pNetData->pTimerRequest = (struct timerequest *)CreateIORequest(pNetData->pTimerPort, sizeof(*pNetData->pTimerRequest));
                if (pNetData->pTimerRequest)
                {
                    if (OpenDevice((CONST_STRPTR)"timer.device", UNIT_MICROHZ, (struct IORequest *)pNetData->pTimerRequest, 0) == 0)
                    {
                        pNetData->pTimerRequest->tr_node.io_Command = TR_ADDREQUEST;
                        pNetData->pTimerRequest->tr_time.tv_secs = 1;
                        pNetData->pTimerRequest->tr_time.tv_micro = 0;
                        SendIO((struct IORequest *)pNetData->pTimerRequest);
                        AbortIO((struct IORequest *)pNetData->pTimerRequest);

                        return pNetData;
                    }

                    DeleteIORequest((struct IORequest *)pNetData->pTimerRequest);
                }

                DeleteMsgPort(pNetData->pTimerPort);
            }
...
void SysNetShutdown(struct SysNetData *pNetData)
{
	if (!pNetData)
	{
		return;
	...
	WaitIO((struct IORequest *)pNetData->pTimerRequest);
    CloseDevice((struct IORequest *)pNetData->pTimerRequest);
    DeleteIORequest((struct IORequest *)pNetData->pTimerRequest);
    DeleteMsgPort(pNetData->pTimerPort);
    CloseLibrary(SocketBase);
    FreeVec(pNetData);
...
int SysNetResolveName(char *pName)
{
	struct hostent *pRemote;
	int            ip;
    ...
    pRemote = gethostbyname(pName);
    if (pRemote)
    {
    	CopyMem(pRemote->h_addr, &ip, sizeof(int));
    }
    else
    {
    	ip = inet_addr(pName);
...
struct SysSocket *SysNetCreateSocket(int domain, int type, int protocol)
{
    int                r;
    int                one = 1;
	struct SysSocket   *pSocket;

    pSocket = AllocVec(sizeof(*pSocket), MEMF_ANY);
    if (pSocket)
    {
        //pSocket->s = socket(AF_INET, SOCK_STREAM, IPPROTO_IP);
        pSocket->s = socket(domain, type, protocol);
        if (pSocket->s != INVALID_SOCKET)
        {
            r = IoctlSocket(pSocket->s, FIONBIO, (void *)&one);
            if (r == 0)
            {
                return pSocket;
            }
...
void SysNetDeleteSocket(struct SysSocket *pSocket)
{
	if (!pSocket)
	{
		return;
	...
	shutdown(pSocket->s, 0);
	CloseSocket(pSocket->s);
    FreeVec(pSocket);
}

bool SysNetBindAndConnect(struct SysSocket *pSocket, unsigned int address, unsigned short port)
{
	int 			    r;
    struct sockaddr_in 	addr;

    if (!pSocket || !address || !port)
    {
    	return false;
    }
    ...
    r = bind(pSocket->s, (struct sockaddr *)&addr, sizeof(addr));
    if (r != INVALID_SOCKET)
    {
    	addr.sin_family = AF_INET;
	    ...
	    r = connect(pSocket->s, (struct sockaddr *)&addr, sizeof(struct sockaddr_in));
...
int SysNetSend(struct SysSocket *pSocket, const void *pData, int datalen)
{
	int r;

	if (!pSocket || !pData || datalen <= 0)
	{
		return 0;
	}

    r = send(pSocket->s, pData, datalen, 0);
    if (r == -1)
    {
...
int SysNetReceive(struct SysSocket *pSocket, void *pData, int datalen)
{
	int r;
    ...
    r = recv(pSocket->s, pData, datalen, 0);
    if (r == -1)
    {
        if (Errno() == EWOULDBLOCK)
        {
...
int SysNetWait(struct SysNetData *pNetData, bool read, int socket, unsigned int timeout_sec, unsigned int timeout_usec)
{
	int	    ret;
	fd_set  rfds;
    ULONG   sigmask;
    ...
    if (SetSignal(0, 0) & (1 << pNetData->pTimerPort->mp_SigBit))
    {
        Wait(1 << pNetData->pTimerPort->mp_SigBit);
	}
    ...
    pNetData->pTimerRequest->tr_node.io_Command = TR_ADDREQUEST;
    pNetData->pTimerRequest->tr_time.tv_secs = timeout_sec;
    pNetData->pTimerRequest->tr_time.tv_micro = timeout_usec;

    SendIO((struct IORequest *)pNetData->pTimerRequest);

    sigmask = 1 << pNetData->pTimerPort->mp_SigBit;

    if (read)
    {
        ret = WaitSelect(socket + 1, &rfds, 0, 0, 0, &sigmask);
    }
    else
    {
        ret = WaitSelect(socket + 1, 0, &rfds, 0, 0, &sigmask);
    }
